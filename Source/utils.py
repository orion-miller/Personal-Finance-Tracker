...
def calc_metrics(self):
    ...
    for sheet in self.ps.db[self.ps.year_sel][self.ps.month_sel]["ie"]:
        ...
        for cat in self.ps.income_types:
            catlist = ie["Type"]
            self.ps.db[self.ps.year_sel][self.ps.month_sel]["ie_cat"][f"income: {cat}"] = 0  

        for cat in self.ps.expense_types:
            catlist = ie["Type"]
            self.ps.db[self.ps.year_sel][self.ps.month_sel]["ie_cat"][f"expense: {cat}"] = 0            
    ...
    for sheet in self.ps.db[self.ps.year_sel][self.ps.month_sel]["ie"]:
        ...
        for cat in self.ps.income_types:
            ...
            if not indices.empty:
                self.ps.db[self.ps.year_sel][self.ps.month_sel]["ie_cat"][f"income: {cat}"] += np.sum(ie["Amount"].loc[indices]) 

        for cat in self.ps.expense_types:
            catlist = ie["Type"]
            indices = catlist[catlist == cat].index #also ensure values are negative
            if not indices.empty:            
                self.ps.db[self.ps.year_sel][self.ps.month_sel]["ie_cat"][f"expense: {cat}"] += np.sum(ie["Amount"].loc[indices]) 
