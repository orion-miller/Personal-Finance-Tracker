...
if ischar(FileNames)
    FileNames = {FileNames}; %Keep things consistently in a cell still if only one file selected
end

EXP = []; %Structure to store expenses

try
    cd(FilePath)
catch
    return %User hit cancel on uifilebox
end
